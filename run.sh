...
echo "run: ./ci.exe 16_coords.coord -o $DIR/$C16OUT" >> $LOG
./ci.exe 16_coords.coord -o $DIR/$C16OUT >> $LOG
echo "" >> $LOG
echo "run compare.exe to compare with the standard answer:" >> $LOG
./compare.exe cout_16.dat $DIR/$C16OUT >> $LOG
echo "" >> $LOG

echo "run: ./ci.exe 4096_coords.coord -o $DIR/$C4096OUT" >> $LOG
./ci.exe 4096_coords.coord -o $DIR/$C4096OUT >> $LOG
echo "" >> $LOG
echo "run compare.exe to compare with the standard answer:" >> $LOG
